...
export default function CotizacionesView() {
    ...
    useEffect(() => {
        const cargarDatos = async () => {
            const [resClientes, resDestinos, resProveedores, resServicios] = await Promise.all([
                fetch('backend-anda.railway.internal/api/clientes'),
                fetch('backend-anda.railway.internal/api/destinos'),
                fetch('backend-anda.railway.internal/api/proveedores'),
                fetch('backend-anda.railway.internal/api/servicios'),
            ])
    ...
    const buscarCotizaciones = async () => {
        const res = await fetch(`backend-anda.railway.internal/api/ia/cotizaciones?clienteId=${clienteSeleccionado}`)
        const data = await res.json()
        setCotizaciones(data)
    }

    const cargarCotizacion = async (id) => {
        const res = await fetch(`backend-anda.railway.internal/api/ia/cotizaciones/${id}`)
        const data = await res.json()
    ...
    const guardar = async () => {
        ...
        const res = await fetch(`backend-anda.railway.internal/api/ia/cotizaciones/${seleccionada.id}`, {
            method: 'PUT',
